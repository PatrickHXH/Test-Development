...
import ExtractApi from "../../requests/extract.js";
...
export default {
  ...
  methods: {
    ...
    changeproject(value) {
      ...
      this.extractlistreq.project_id = value;
      this.initReportList();
    ...
    async initReportList() {
      ...
      const resp = await ExtractApi.getExtractList(this.extractlistreq);
      if (resp.success === true) {
        this.extractData = resp.items;
        this.extractotal = resp.total;
        // this.$message.success("查询成功");
      } else {
        this.$message.error("查询失败!");
      }
